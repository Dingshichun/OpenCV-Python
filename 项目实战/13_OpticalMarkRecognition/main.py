...
while True:
    ...
    try:
        ...
        biggestPoints = utils.getCornerPoints(
            rectCon[0]
        )  # GET CORNER POINTS OF THE BIGGEST RECTANGLE
        gradePoints = utils.getCornerPoints(
            rectCon[1]
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            cv2.drawContours(
                imgBigContour, biggestPoints, -1, (0, 255, 0), 20
            ...
            cv2.drawContours(
                imgBigContour, gradePoints, -1, (255, 0, 0), 20
            )  # DRAW THE BIGGEST CONTOUR
            ...
            # DISPLAYING ANSWERS
            utils.showAnswers(
                imgWarpColored, myIndex, grading, ans
            ...
            utils.showAnswers(
                imgRawDrawings, myIndex, grading, ans
            )  # DRAW ON NEW IMAGE
            invMatrix = cv2.getPerspectiveTransform(
                pts2, pts1
            )  # INVERSE TRANSFORMATION MATRIX
            imgInvWarp = cv2.warpPerspective(
                imgRawDrawings, invMatrix, (widthImg, heightImg)
            )  # INV IMAGE WARP

            # DISPLAY GRADE
            imgRawGrade = np.zeros_like(
                imgGradeDisplay, np.uint8
            ...
            cv2.putText(
                ...
                (0, 255, 255),
                3,
            )  # ADD THE GRADE TO NEW IMAGE
            invMatrixG = cv2.getPerspectiveTransform(
                ptsG2, ptsG1
            )  # INVERSE TRANSFORMATION MATRIX
    ...
    if cv2.waitKey(1) & 0xFF == ord("s"):
        cv2.imwrite("./ScannedImage" + str(count) + ".jpg", imgFinal)
        cv2.rectangle(
            stackedImage,
            (
